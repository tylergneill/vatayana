...
global doc_explore_inner_results_html_template, doc_compare_inner_results_html_template, \
    topic_adjust_inner_results_html_template, text_prioritize_inner_html_template
...
stored_text_view_html_full_path = os.path.join(CURRENT_FOLDER, stored_text_view_html_relative_path)

stored_text_view_html_fns = [os.path.join(stored_text_view_html_full_path, html_fn)
...
def format_text_view(text_abbreviation):
    ...
    for h2 in h2s:
        ...
        relevant_work_doc_ids = [
            doc_id for doc_id in work_doc_ids
            if parse_complex_doc_id(doc_id)[1] == h2
        ]
        links_addendum += '  '.join(
            ["(<a href='docExplore?doc_id={}'>{}</a>)".format(doc_id, doc_id) for doc_id in relevant_work_doc_ids])
        links_addendum += "</small></small>"
...
def get_text_view(text_abbreviation):
    ...
    view_text_html_fn_full_path = os.path.join(stored_text_view_html_full_path, view_text_html_fn)
...
def get_top_topic_indices(doc_id, max_n=5, threshold=0.03):
    ...
    indices_of_dominant_n_topics = indices_of_top_n_elements(input_list=thetas[doc_id], n=max_n)
...
def rank_all_candidates_by_topic_similarity(query_id):
    ...
    for doc_id in doc_ids:
        ...
        if doc_fulltext[doc_id] == '':
            ...
            topic_similarity_score[doc_id] = round(1 - fastdist.cosine(query_vector, candidate_vector), 4)
    ...
    for work_name, work_doc_ids in doc_ids_grouped_by_work.items():
        ...
        for doc_id in work_doc_ids:
            ...
            if doc_id in cumulative_results_for_this_work.keys():
                ...
                cumulative_results_for_this_work[doc_id] = rank_candidates_by_tiny_tf_idf_similarity(
                    doc_id,
                    ids_for_closest_n_docs_by_topics
                )
...
def format_similarity_result_columns(query_id, priority_results_list_content, secondary_results_list_content):
    ...
    priority_col_html += ''.join([
        table_row_template % (
            ...
            format_doc_compare_link(query_id, doc_id, display_string=results[2][1][:25], title=results[2][1]),
        )
...
def get_closest_docs(query_id,
    ...
    if doc_fulltext[query_id] == '':
        results_html = html_templates['docExploreInner'].substitute(
            query_id=query_id, query_section=section_labels[query_id], prev_doc_id=doc_links[query_id]['prev'],
            next_doc_id=doc_links[query_id]['next'],
            query_original_fulltext=doc_original_fulltext[query_id], query_segmented_fulltext='', top_topics_summary='',
            priority_results_list_content='', secondary_results_list_content='', priority_texts=str(priority_texts),
    ...
    for k, score_phrase_pair in sw_w_alignment_candidates.items():
        if score_phrase_pair[0] == 0.0:
            ...
            sw_w_alignment_candidates[k] = (str(sw_w_alignment_candidates[k][0]), sw_w_alignment_candidates[k][1])
    ...
    if results_as_links_only:
        similarity_result_doc_links = list_to_linking_dict(list(priority_ranked_results_complete.keys()))
...
def format_batch_result_rows(results: List[Dict[str, Union[str, float]]], priority_texts):
    ...
    for i, result in enumerate(results):
        ...
        """.format(
            ...
            result['tf_idf'],
            result['sw_w'],
            format_doc_compare_link(result['query_id'], result['doc_id_2'], result['sw_w_phrase'], title=""),
        )
    return html_rows
...
def compare_doc_pair(doc_id_1, doc_id_2,
    ...
    doc_1_topic_vector = np.array(thetas[doc_id_1])
    doc_2_topic_vector = np.array(thetas[doc_id_2])
    topic_similarity_score = round(1 - fastdist.cosine(doc_1_topic_vector, doc_2_topic_vector), 4)
...
def format_search_depth_output(n_tf_idf_shallow, n_sw_w_shallow, n_tf_idf_deep, n_sw_w_deep, priority_texts,
...
""".format(topic_secs_per_comparison, tf_idf_secs_per_comparison, sw_w_secs_per_comparison,
           num_docs * topic_secs_per_comparison)

    shallow_slider_html, shallow_slider_js = format_search_depth_slider_pair(n_tf_idf_shallow, n_sw_w_shallow,
                                                                             priority_texts, depth='shallow')
    deep_slider_html, deep_slider_js = format_search_depth_slider_pair(n_tf_idf_deep, n_sw_w_deep, priority_texts,
                                                                       depth='deep')
