...
app.config[
    "SECRET_KEY"
] = "safaksdfakjdshfkajshfka"  # for session, no actual need for secrecy
MONGO_CRED = open("mongo_cred.txt").read().strip()
# app.config["MONGO_URI"] = "mongodb://localhost:27017/my_db"
app.config[
    "MONGO_URI"
] = f"mongodb+srv://tyler:{MONGO_CRED}@sanskrit2.urlv9ek.mongodb.net/vatayana?retryWrites=true&w=majority"
...
def doc_explore():
    ...
    if request.method == "POST" or "doc_id" in request.args:
        ...
        if (
            doc_id in ["", None]
            and local_doc_id_input in ["", None]
            and local_doc_id_input_2 in ["", None]
        ):
        ...
        if (
            ...
            or (
                ...
                and (IR_tools.doc_ids.index(doc_id) < IR_tools.doc_ids.index(doc_id_2))
            )
        ):
            ...
            if doc_id_2 != "":
                ...
                best_results = IR_tools.batch_mode(
                    similarity_data, doc_id, doc_id_2, sw_threshold
                )
...
def doc_compare():
    ...
    if request.method == "POST" or "doc_id_1" in request.args:
        ...
        if doc_id_1 == doc_id_2:
            ...
            (
                doc_compare_inner_html,
                sim_btn_left,
                sim_btn_right,
            ) = IR_tools.compare_doc_pair(
                doc_id_1,
                doc_id_2,
                topic_labels=session["topic_labels"],
                priority_texts=session["priority_texts"],
            ...
            doc_compare_inner_html = (
                f"<br><p>Please enter two valid doc ids like "
                f"{str(IR_tools.ex_doc_ids)[1:-1]} etc.</p>"
...
def text_view():
    ...
    if (
        request.method == "POST"
        or "text_abbrv" in request.args
        or "doc_id" in request.args
    ):

        if "doc_id" in request.args:

            doc_id = request.args.get("doc_id")
            text_abbreviation_input, local_doc_id_input = IR_tools.parse_complex_doc_id(
                doc_id
            )
            return redirect(
                "/textView?text_abbrv={}#{}".format(
                    text_abbreviation_input, local_doc_id_input
                )
            )
            ...
            if local_doc_id_input != "":
                ...
                (
                    text_abbreviation_input,
                    local_doc_id_input,
                ) = IR_tools.parse_complex_doc_id(doc_id)
            return redirect(
                "/textView?text_abbrv={}#{}".format(
                    text_abbreviation_input, local_doc_id_input
                )
        ...
        if text_abbreviation_input in disallowed_fulltexts:
            ...
            text_html = (
                "<br><p>sorry, fulltext is not available for these texts at present: "
            )
            ...
            text_html += (
                " etc.</p><p>See <a href='assets/doc_id_list.txt' target='_blank'>doc id list</a> "
...
def brucheion_align():
    ...
    brucheion_html_body_full_fn = os.path.join(
        CURRENT_FOLDER, relative_path_to_brucheion_html_body_fn
    )
    ...
    if request.method == "POST":
        ...
        for key, val in request.form.items():
            ...
            if key == "prioritize_all_texts":
                ...
                priority_texts_input = (
                    all_texts[: all_texts.index(one_text)] + priority_texts_input
                )

            elif key == "prioritize_later_checkbox":  # checkbox

                priority_texts_input = (
                    priority_texts_input + all_texts[all_texts.index(one_text) + 1 :]
                )
    ...
    if request.method == "POST":
        ...
        for key, val in request.form.items():
            ...
            if key == "n_tf_idf_shallow_slider":
                ...
                session["n_tf_idf_shallow"] = IR_tools.search_n_defaults[
                    "n_tf_idf_shallow"
                ]
                session["n_tf_idf_deep"] = IR_tools.search_n_defaults["n_tf_idf_deep"]
                session["n_sw_w_shallow"] = IR_tools.search_n_defaults["n_sw_w_shallow"]
